...
def plot_rate_and_packetloss_graph(rates:list,packet_loss:list,filename:str = 'UDP Traffic'):
...
if __name__ == '__main__':
    ...
    #     find_files(dir,base)
    file_matches = glob.glob(filenames, recursive=True)
    ...
    for filename in file_matches:
        if os.path.isfile(filename) and os.access(filename, os.R_OK):
            ...
            p = Process(target=plot_rate_and_packetloss_graph,args=(y1,y2,filename,))
            ps.append(p)
        else:
